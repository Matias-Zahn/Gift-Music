...
function ArtistDetail() {
  ...
  return (
    <PrincipalLayout artistListBug>
      <Link className="flex gap-2 text-secondary" to={-1}> <ArrowLeftIcon /> Atras </Link>
      <header className="flex flex-col md:flex-row justify-center items-center gap-8 p-10 font-bold">
        ...
        <div className="grid gap-2 place-items-center md:place-items-baseline">
          ...
          <h5>
            Seguidores: <span>{(artistInfo?.followers.total / 1000).toFixed(0)}K</span>{" "}
          </h5>
          ...
          <ul className="flex text-sm flex-wrap gap-2 uppercase">
            {
              artistInfo?.genres.map((genre) => <li className="border p-2 px-3 rounded-3xl" key={genre}>{genre}</li>)
            }
          </ul>
        </div>
      </header>
          ...
          {artistInfo?.albums.map((album) => (
            <SwiperSlide key={album.id}>
              <article className="text-sm grid gap-[2px]">
                <header className=" rounded-xl max-w-[150px]">
      ...
      <section className="mt-5">
        {
          artistInfo?.songsTop.map((song) => <TrackCard key={song.id} track={song} /> )
        }
      </section>
    </PrincipalLayout>
  );
